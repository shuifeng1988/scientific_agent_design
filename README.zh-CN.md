# Scientific Research Agent Design：自动化科研项目执行与报告

[英文说明](README.en.md)

本项目是一套面向真实科研项目的自动化科研 Skill 与 Supervisor Agent 框架，将研究问题定义、文献调研、方案设计、软件与数据准备、任务执行、质量控制、结果解释和后续研究组织成连续的科研工作流程。它帮助研究人员从总体目标出发，建立清晰的研究计划，管理执行进度，并积累可审计、可复现、便于人工阅读的研究成果。

其中，Skill 定义科研工作的具体规范，指导 Agent 理解问题、制定计划、按步骤执行、记录证据并解释结果；Supervisor 根据已确认的计划、任务依赖、可用资源和执行状态协调任务，跟踪进度、管理故障恢复并推进后续工作。两者共同将项目目标、执行过程和成果报告连接起来。

## 1. 核心优势：像研究人员一样组织科研，按计划逐步交付成果

本项目的核心理念是**像研究人员一样组织科研：先定义好问题，经过调研和讨论规划好研究步骤，再由 Skill 指导 Agent 严格按照已确认的计划一步一步执行，并对结果进行解释与反思**。执行以 `01_PLAN.md` 为依据，在 `results/` 中为每一步形成一一对应、可复核的研究成果包。研究人员沿着计划，即可找到该步骤的数据来源、软件与方法、实际命令、结果、结论、科学解释、图表和 Word 报告。需要调整研究方向时，先讨论并记录计划修订，再按更新后的步骤继续。

`00_PROJECT.md` 定义科学问题，`01_PLAN.md` 将问题分解为可执行任务，`registry/tasks.tsv` 为每个任务登记唯一结果路径。多个运行、模型或种子子任务保存在该任务目录内，汇总到同一任务报告。一般科研项目无需强行展开成模型比较矩阵；比较研究则必须覆盖计划批准的全部对象。

下面是目录组织示例，不是已经运行的实验结果：

```text
00_PROJECT.md：Q01 某因素是否影响研究指标？
01_PLAN.md：Q01.01 数据质量检查；Q01.02 主要比较
registry/tasks.tsv：登记每个任务的唯一 result_path
results/
└── Q01_research_question/
    ├── Q01.01_data_quality/
    │   ├── README.md          # 数据、方法、结果、结论与解释
    │   ├── REPORT.docx        # 与 Markdown 结论一致的 Word 报告
    │   ├── tables/            # 统计、缺失记录与失败表
    │   ├── figures/           # 图形、图例和图注
    │   └── _evidence/         # 运行记录、质控、哈希与解释证据
    └── Q01.02_primary_analysis/
        ├── README.md
        ├── REPORT.docx
        ├── tables/
        ├── figures/
        └── _evidence/
```

| 成果包内容 | 具体要求 |
|---|---|
| 计划对应关系 | 科学问题、任务编号、计划版本、依赖和预期；链接回项目与计划 |
| 数据来源 | 数据介绍、来源链接、版本、获取或调用方法、实际路径、筛选条件、样本数和校验值；大型原始数据可引用，不重复拷贝 |
| 软件与方法 | 软件用途、来源、版本、环境、权重版本、参数、实际命令、种子、主机与 CPU/GPU 资源 |
| 结果 | 数值、样本分母、效应量及适用的不确定性指标；完整记录缺失、失败和已批准排除 |
| 表与图 | 可读取结果表、对应图形与图注、来源和生成方法；图中数据可追溯到表与运行 |
| 结论 | 回答本步骤的科学问题，明确证据支持什么、不支持什么 |
| 解释与反思 | 预期与观察是否相符、可能机制、替代解释、文献支持或冲突、局限和下一步影响 |
| 人工可读报告 | `README.md` 与 `REPORT.docx` 使用一致的数值、图表和结论；不能只留下日志 |
| 验收证据 | 运行编号、输入输出哈希、质量检查、失败记录和审阅决定 |

每批结果验证后更新对应表图与报告草案，正式分析完成时交付一致的 Markdown 与 Word 报告。中间数据节点可在哈希与质控通过后供后续计算使用；其所属正式分析仍需完成报告、解释和项目事件验收。图表、Word 和科学解释由实际配置的分析与报告工作程序生成；守护进程负责调度和结构检查，不能替代科学审阅。

## 2. 从研究问题到执行的架构

```mermaid
flowchart TD
  U["用户提出科研项目或问题"] --> A["分轮澄清、等待回答并确认方向"]
  A --> B["初步 00_PROJECT.md 与 01_PLAN.md"]
  B --> C["深度研究：进展、意义、可行性与方法"]
  C --> D["用户讨论、修订与确认"]
  D --> E["建立五个正式根文件并冻结研究设计"]
  E --> P["01_PLAN.md 的具体步骤"]
  P --> R["登记编号、唯一结果目录、依赖与资源"]
  R --> S["Supervisor 调度就绪任务"]
  S --> X["按实际主机配置执行"]
  X --> V["核对结果与质量"]
  V --> O["results 中对应任务的成果包"]
  O --> I["方法、结果、解释、图表与 Word 验收"]
  I --> H["事件记录与文档状态更新"]
  H --> N{"下一步科学判断"}
  N -->|"继续或登记补充证据任务"| R
  N -->|"重大异常或设计变化"| D
  X -->|"技术失败"| F["保存证据、诊断、修复与最小验证"]
  F -->|"仅恢复失败阶段"| X
```

五个正式根文件是经讨论确立的项目基线。软件或数据尚未就绪时据实标记；状态持续更新，研究设计修改保留审批与版本记录。“正式”不表示安装、下载或研究已经完成。

| 文件 | 用途 |
|---|---|
| `00_PROJECT.md` | 项目目的、科学问题、范围、假设和最终交付 |
| `01_PLAN.md` | 每一步的数据、软件、方法、依赖、结果路径和完成标准 |
| `02_SOFTWARE.md` | 软件与模型介绍、来源、版本、环境、安装使用方法、主机位置与验证 |
| `03_DATA.md` | 数据介绍、来源版本、下载调用方法、存储位置、校验和划分 |
| `04_STATUS.md` | 待执行、进行中、失败、阻断、完成状态，以及证据链接和下一步 |

## 3. 核心 Skill 的逐步工作流

1. **理解目的。** 不要求用户第一条消息就写清所有需求。面对新项目或含糊方向，先简短复述理解，每轮询问 1–3 个关键问题，然后结束回复、等待回答；结合反馈继续澄清和思考备选方向。汇总目的、具体问题、范围、已有证据/资源和预期用途，经用户确认后才进入初步计划。不能提问后又在同一回复中直接给出完整方案。已批准任务的续跑不重复接洽。
2. **建立初步计划。** 形成草案 `00_PROJECT.md`、`01_PLAN.md`，保留假设、候选方法和待决问题。
3. **深度研究。** 检索基础与近期进展，记录日期、来源、查询和证据；比较常用、当前可复现和新方法，评价意义、是否值得做、可行性与信息增益。近期窗口通常覆盖过去 24–36 个月，并保留基础文献。
4. **讨论后定案。** 展示方法选项、风险、数据与资源需求；用户确认后建立五个正式根文件，登记统计、排除、停止和修订规则。
5. **按步骤准备。** 从 `01_PLAN.md` 选择依赖已满足的步骤，读取其数据、软件、命令、结果路径和验收要求。
6. **执行与记录。** 在对应目录保存数据引用、实际方法和运行证据，逐批形成表图、结果与报告，避免结果和计划失联。
7. **质控与交付。** 核对行数、分母、哈希、环境和失败机制；完成该步骤的 Markdown、Word 与图表，不通过时明确标记原因。
8. **解释与反思。** 回答任务问题，对比预期、文献支持与冲突，记录替代解释、局限及后续分析的意义。
9. **恢复或修订。** 技术失败先诊断，再有界修复与阶段续跑；重大科学异常和冻结设计变化交由用户讨论。范围内低风险且有价值的补充分析登记为可追踪任务。
10. **更新并推进。** 项目事件记录下载、环境、运行、质控和修订，更新根文件与状态；已授权、依赖满足且资源可用的后续任务继续执行。

[深度研究门槛规范](skills/scientific-research-project/references/deep_research_gate.md)解释第 3–4 步如何检索、评价和记录用户决定，它是内部工作规范，不是版本文件。交付要求见[项目与成果包规范](skills/scientific-research-project/references/project_schema.md)。

### 3.1 项目论证、详细步骤与深度调研的质量要求

五个根文件不是五张填空表。初步草案允许列出待决项；正式方案必须形成“已有证据 → 具体空白 → 为什么重要 → 研究步骤 → 产生什么数据 → 能支持或反驳什么”的完整论证。

| 文档/阶段 | 必须展开的内容 | 不足以验收的输出 |
|---|---|---|
| 00_PROJECT.md | 背景与最相近研究、支持及反证、具体知识空白、科学/实际意义、贡献边界、零结果价值、假设与可行性 | 一段目标、几个假设、没有来源的“创新且重要” |
| 01_PLAN.md | 总表之外，为每个任务及必要子步骤写清目的、输入数据、完整比较对象、方法/对照/统计、命令或待实现接口、输出结构与路径、图表、质控、证据用途和后续分支 | “清洗数据—建模—验证—写报告”式列表，只有软件名或结果目录 |
| 深度调研 | 分主题检索与筛选、核读关键原文/方法/图表、最新与矛盾证据、方法比较、数据可行性、跨研究综合和方案取舍 | 只取第一批搜索结果、堆论文摘要、只核对 DOI 存在、原始搜索文件未汇总进方案 |

每一步按“行动 → 输入 → 处理/命令 → 输出字段与路径 → 质控 → 证据用途”描述。输出用于检验假设，不预先承诺证明偏好的结论。未开发脚本必须注明待实现；科学上的关键选择须讨论，不能为填表虚构参数和阈值。

调研先形成按问题组织的证据矩阵、最相近工作与空白比较、方法/数据可行性分析，再写连贯的研究意义和详细方案。提交用户审批前进行实质审阅，指出具体缺项与修正；不是用字数、论文数量或模板字段判定深度。新版冻结程序会拒绝明确标为 preliminary/incomplete 等未就绪状态的调研，但仍保留旧格式兼容；程序不会判断论证本身是否充分，旧冻结项目也不会被自动撤销。

详见[科研设计深度规范](skills/scientific-research-project/references/research_depth.md)、[项目模板](project_template/00_PROJECT.md)和[详细计划模板](project_template/01_PLAN.md)。仅修订模板不会自动重写已有项目；已冻结设计仍需审阅和批准修订。

### 3.2 设计借鉴与来源致谢：Skill / Agent

| 借鉴来源 | 具体采用的设计 | 保留的边界 |
|---|---|---|
| [Cheng-I Wu：academic-research-skills](https://github.com/Imbad0202/academic-research-skills)，本机 academic-research-suite 0.1.15 | 问题与方法匹配、来源核验、跨研究综合、反证审阅 | 不搬入第二套目录或自动多智能体团队，不按论文数量投票 |
| [K-Dense：literature-review](https://github.com/K-Dense-AI/scientific-agent-skills)，本机 1.2 | 分阶段检索、去重筛选、全文取证与引用核验 | 不强制特定付费工具、生成式图片或期刊声望筛选 |
| 主机提供的 deep-research-work，本机 0.1.15 | 选项式澄清、追查检索线索、完整研究综合 | 可用性取决于宿主平台，不复制平台专有工具与文件 |

这里注明的是已检查的本机版本，不代表上游最新版本，也不声称已对三套框架完成效果评测。

科研 Agent 框架层面的设计参考还包括 Robin 和 Google Co-Scientist；我们借鉴研究规划、证据反馈和批判性审阅的思路，结合本项目的计划—成果一一对应规范独立实现。二者的原始论文和定位比较见第 5 节。这不是代码复用、官方合作或性能优越性的声明。

已比较本地 academic-research-suite、literature-review 与主机 deep-research 的相关设计，采用问题与方法匹配、分阶段取证、主题综合和反证审阅等方法，形成[科研技能协作规范](skills/scientific-research-project/references/research_skill_bridge.md)。可用时按阶段使用外部 Skill；未安装时执行本项目自含规范，不要求安装整套新框架。

总体目标、五个根文件、审批、任务路径与调度仍由本框架管理；外部 Skill 不另建总计划或平行成果目录。同一 Agent 的不同审阅阶段不冒充独立多智能体审查。本次未复制第三方原文、代码或模板：ARS 本地许可有非商业限制，其他资源的再分发条款也不能只凭署名假定。来源、版本、已读文件哈希及采用/不采用理由见[双语借鉴记录](sources/RESEARCH_SKILL_COMPARISON.md)。


### 3.3 最核心的科学证据来自哪里

每个项目的 `00_PROJECT.md` 必须直接回答这个问题，不能让读者从几十篇引用或附件里自行猜测。核心证据是能区分主要假设与竞争解释的测量、比较或推导，不一定是规模最大或发表期刊最好的数据。

| 证据层次 | 必须明确的来源与用途 | 不能混淆的事项 |
|---|---|---|
| 已有研究依据 | 原始研究、数据编号/版本、具体方法及图表位置；支撑研究意义和方法选择 | 综述、搜索摘要和 Skill 署名不能替代原始科学证据 |
| 本项目决定性证据 | 实际输入数据、独立重复单位、关键测量、对照、任务/运行及结果表图；直接检验核心假设 | 未运行的是计划，不是结果；公共数据重分析仍须注明外部采集来源 |
| 独立验证与反证 | 独立样本/队列、正交终点、负对照及矛盾结果；检验结论边界 | 同一批样本产生的两篇论文不能自动算独立验证；相关性不能直接证明机制 |

在 00 中列出“主张/问题 → 原始来源 → 测量和独立单位 → 决定性比较 → 01 中的任务 → 结果表图 → 能/不能支持的结论”。01 详细规定如何获得这些证据；03 保存数据获取与版本记录；运行后在对应成果包中补上真实分母、失败/排除、效应与不确定性。文献综述项目以原始研究的提取证据为主体；模拟和理论项目说明其假设与证明边界，不虚构实验数据。

Agent 必须说明：如果最关键证据缺失、对照不成立或观察结果相反，哪些结论不再成立、哪些任务需要调整。外部文献结果、本项目观察和 Agent 推断分开标记。详见[核心证据规范](skills/scientific-research-project/references/core_evidence.md)。这些是实际写作与审阅义务，不是文件哈希或程序检查能够自动保证的科学正确性。

## 4. Skill 与 Supervisor 的职责

| 部件 | 职责与边界 |
|---|---|
| 核心 Skill | 指导按计划执行、维持任务与成果一一对应、生成报告并解释科学意义 |
| Supervisor 命令行工具 | 检查需求、项目、任务与报告结构；一次检查不等于开始后台运行 |
| 常驻 Supervisor | 按已配置依赖、执行契约与资源派发任务，跟踪作业、校验产物与管理重试 |
| 分析、报告和解读工作程序 | 调用领域工具，生成表图、Word 与科学解释；需要真实命令、环境和访问权限 |
| 项目事件与状态机制 | 关联任务、运行和证据，记录失败与修订，维护人工入口 |

文件存在性、关键字或 Skill 格式检查均不能证明科学结论正确。完成标准需要实际证据与审阅。

## 5. 与 Robin、Co-Scientist 的定位比较

下表比较工作组织重点，不是性能排名。Robin 已包含数据分析和实验反馈，不能将它描述为仅生成假设。本项目一栏描述自身协议和实现边界。

| 维度 | 本项目 | Robin | Google Co-Scientist |
|---|---|---|---|
| 组织目标 | 按研究计划执行并逐步交付可追溯成果 | 连接文献、假设、实验数据分析和下一轮假设 | 根据研究目标生成、评议与改进假设 |
| 人工检查入口 | 五个根文件；每项计划任务对应唯一结果目录和报告 | 论文展示实验计划、数据分析与反馈迭代 | 论文展示研究提案与假设迭代 |
| 科学反馈 | 结果与预期、文献对照，决定继续、补证据或修订 | 根据实验结果更新假设 | 通过多智能体评议和竞赛演化改进假设 |
| 本项目强调的格式 | 每步的数据来源、方法、结果、结论、解释、表图与 Word | 不据论文推断其采用本仓库目录和交付约定 | 不据论文推断其采用本仓库目录和交付约定 |

依据：[Robin 原始论文](https://arxiv.org/abs/2505.13400)、[Co-Scientist 论文](https://arxiv.org/abs/2502.18864)。本仓库尚无与二者直接比较的性能实验，也未提供二者的即用集成适配。

## 6. 当前执行能力与限制

内置驱动是 **Linux 本地或 SSH 远程的用户级 systemd 驱动**。主机需要 Python、可用的用户服务管理器和任务环境；远程还需可用的 SSH 登录。GPU 探测通过 `nvidia-smi` 获取 NVIDIA GPU 信息，不能宣称所有显卡均已验证。

| 范围 | 当前代码情况 |
|---|---|
| 本地任务 | 驱动有本地启动与探测路径；调度器仍要求 `host=local` 的任务显式声明 `lightweight: true`，不宣称本地 GPU 重计算已通用放开 |
| SSH 服务器 | 驱动可管理用户级 systemd 作业；使用 CPU/GPU 前须按具体环境验证资源和任务命令 |
| 容器、Slurm、云作业与在线 API 调度 | 无内置专用适配器，不列作当前支持能力 |

通用说明不指定 H100/V100。`bootstrap_supervisor.py` 必须读取用户审查过的 `--hosts` 配置，不再内置个人服务器地址。本次增加的是研究设计与成果包验收，不是新的执行后端或硬件兼容性验证。

### 6.1 工作流要求如何落实与验收

新项目使用 `workflow_policy=plan_results_v1`，执行调度仍使用 `execution_policy=v2`。下表区分程序检查与科研责任，避免把“写了规则”当作“已经做到”。

| README 要求 | 当前落实方式 | 验收范围与边界 |
|---|---|---|
| 先理解问题再建草案 | Skill、默认调用提示和 Claude 适配要求分轮提问、等待回答及方向确认；CLI 不再把关键词当作确认 | 对话质量仍需人工/行为检查；草案工具只创建 00/01，本身不能证明交流已发生 |
| 深度研究后讨论定案 | 冻结前校验两条检索途径、检索证据哈希、方法比较和评价，以及绑定方案与调研的用户决定记录 | 程序不执行检索、不鉴别人类身份，也不判定研究价值；Agent 必须真实检索并取得决定 |
| 五个根文件与计划一一对应 | 冻结版本、任务清单、唯一结果路径；新派发前检查设计与注册表 | 未批准变更阻断新任务；仍核对已经运行的作业，不重复提交 |
| 完整执行与持续推进 | 依赖图、资源预留、阶段命令、持久状态和任务独立恢复额度 | 需真实领域命令、环境、凭证与监控服务；本次未新测远程 GPU 或在线 AI |
| 每步交付可读成果包 | 同一清单生成 Markdown/Word；检查源表内容、内嵌图、运行编号和完整 QC 哈希 | 支持 CSV/TSV 汇总表及 PNG/JPEG 图；分析与制图由领域工作程序完成 |
| 结果解释与反思 | 检查预期、观察、文献记录、替代解释和下一步决定；科学异常可阻断正式发布 | 校验结构与来源不能保证统计正确、图形表达准确或推理可靠，仍须科学审阅 |
| 事件触发文档更新 | 持久事件、幂等重放，更新 02/03/04、任务表与结果索引 | 冻结的 00/01 不随进度改写；质控失败生成修订草案，设计变更仍需经审查迁移 |

回归测试包含真正运行的小型本地子进程：两个依赖任务分别执行、校验、解释，重建调度器后继续，生成真实 Markdown、Word、表和图并验收。输入、检索和审批均明确标记为合成测试；资源驱动与监控被隔离替代，因此这不是实际科研、远程 systemd 或在线 Agent 的端到端验收。具体格式和边界见[工作流执行契约](skills/scientific-research-project/references/workflow_contract.md)。

## 7. 安装与使用

### 7.1 获取源码

尚未克隆时执行：

```bash
mkdir -p "$HOME/git"
git clone https://github.com/shuifeng1988/scientific_agent_design.git "$HOME/git/scientific_agent_design"
```

已有源码时，确认本地修改保存后运行 `git pull --ff-only`。源码目录用于开发，安装副本用于运行。源码使用的 `Path.is_relative_to` 要求 Python 3.9 或更新版本；这只是代码最低要求，并非各版本均已完整测试。

### 7.2 Codex 安装

```bash
research_source="$HOME/git/scientific_agent_design"
research_skill="$research_source/skills/scientific-research-project"
codex_skill="$HOME/.codex/skills/scientific-research-project"
mkdir -p "$codex_skill"
cp -a "$research_skill/." "$codex_skill/"
```

自定义 Codex 用户目录需替换目标路径。新会话使用 `$scientific-research-project`，未发现更新时重开会话。更新前检查安装副本是否有独立修改。

若当前安装包含 Skill 校验器：

```bash
python3 "$HOME/.codex/skills/.system/skill-creator/scripts/quick_validate.py" "$codex_skill"
```

此检查仅验证 Skill 结构，不验证科研分析、Word 生成或服务器运行。

#### 7.2.1 重装成功，为什么已有项目的 PROJECT / PLAN 没变？

源码更新、安装副本更新、项目文档修订、冻结基线迁移是四件不同的事。复制 Skill 或重新启动 Supervisor 不会重新编写已有根文件；`draft` / `freeze` 使用 Agent 提供的内容，不会自动把简短方案扩写为详细科学设计。模板指导写作，也不是自动渲染已有项目的程序。

先核对实际读取的 Skill 路径和文件，再检查项目的 `registry/design.json`、当前 00/01 和已有修订草案。`check` 通过只说明基线与哈希一致，不说明计划足够详细。已经冻结的旧项目需要明确的修订工作：保留原始记录，写出实际新版项目论证和每一步详细方案，展示差异及待决问题，再经所需确认更新基线；不能删除注册表或篡改旧批准记录来绕过检查。当前没有自动应用修订的一键命令。

在目标项目中新一轮明确调用：

```text
$scientific-research-project
请报告实际读取的 Skill 路径，并按 research_depth.md 和 core_evidence.md
审查本项目现有 00_PROJECT.md / 01_PLAN.md 和已有修订草案。
请生成完整的替换草稿，而不是只列修改建议：展开研究意义、核心证据来源，
以及每一步的目的、数据、方法、输出、质控和证据用途。
保留任务编号、已有结果和冻结基线，沿用已有修订目录。
列出差异和确实需要我决定的问题；在批准迁移前不要覆盖正式文件或启动新分析。
```

若已有待审修订，应继续完善该草案，不反复新建“最终新版”。结束时必须说明安装是否验证、草稿在哪里、正式文件是否改变及原因。下一轮可使用新安装的 Skill；仍读到旧规则时重新打开会话。运行中的服务使用自己的脚本路径，不能仅凭安装成功声称其已更新。

### 7.3 Claude Code 安装

在目标科研项目根目录执行；首次安装前确认目标文件没有独立修改：

```bash
research_source="$HOME/git/scientific_agent_design"
mkdir -p .claude/skills/scientific-research-project .claude/agents
cp -a "$research_source/skills/scientific-research-project/." .claude/skills/scientific-research-project/
cp "$research_source/adapters/claude/scientific-supervisor.md" .claude/agents/scientific-supervisor.md
```

重新打开会话调用 Skill 或 Supervisor。这里只安装指令与适配文件；内置 `progress_worker.py` 使用 Codex CLI，不能据此声称 Claude 常驻工作程序已经验收。

### 7.4 开始研究与检查任务

#### 7.4.1 在 Codex 中明确调用

在目标项目中打开 Codex，把下面内容发到**消息输入框，不是 Bash 终端**。先输入 `$` 并选择对应 Skill（若当前客户端提供选择器），或直接明确写出技能名。安装多个科研技能时，不要只依赖自动匹配。

新项目可以只提出一个尚未成熟的问题：

```text
$scientific-research-project

我想研究【大致方向】，但问题还不够具体。
请先简短复述你的理解，每轮问我 1–3 个关键问题并等待回答，
每题优先给出简短选项，保留“其他：自由输入”，让我能否定所有选项并另提方向。
结合我的反馈一起明确科研目的、具体问题、范围和已有条件。
在我确认方向前，不要直接输出完整方案或启动分析。
之后建立初步 00_PROJECT.md / 01_PLAN.md，做深度调研，
再与我讨论方法，确认正式方案后才执行。
```

这种先交流的行为是 Skill 默认要求，不需要用户每次补上提醒。第一轮应是简短理解与问题，不是长清单、任务树或完整报告。用户回答后继续澄清，而不是把一次回答当作所有设计已获批准；方向确认和深度研究后的正式方案确认是两个不同阶段。

已有批准项目使用：

```text
$scientific-research-project

请读取五个根文件、任务注册表和已有批准记录，继续已确认的研究计划。
不要重复已回答的接洽问题，不重跑已验证成功的阶段。
按任务编号在注册的 results 路径保存来源、方法、实际命令、
结果、结论、科学解释、图表和 Word；重大歧义或设计变化再与我讨论。
```

新安装或更新后，在下一轮调用；如仍沿用旧提示，重新打开会话。可要求 Agent 报告实际读取的 `SKILL.md` 路径和首轮规则，避免调用同名旧副本。仓库名 `scientific_agent_design` 不是技能名，也不存在本仓库安装后自动提供的 `$scientific-supervisor` Codex 技能。

澄清优先采用“选项 + 自由输入”，而不是让用户从零写完整需求。每题通常提供 2–3 个易区分的选项；适当时加入“还没想清楚，请帮我梳理”，并始终保留“其他：自己描述”。用户可以直接选项、补充一句话，或完全推翻选项重新表达。界面自带“其他/自由输入”时不重复添加；遵守实际单/多选和选项数量限制，默认选中不算用户确认。优先使用可用且允许的交互提问工具，无工具时按界面规则提供文字选项或简短开放问题，不假称已实现可点击界面。精确靶点、文件路径等需要自由输入的细节，只问缺少的那一项。

#### 7.4.2 在 Claude Code 中调用

这里指 Claude Code，不是普通 Claude 网页聊天。完成 7.3 安装后，在会话中调用：

```text
/scientific-research-project 我想研究【方向】，请先与我分轮澄清，等待回答后再规划。
```

要调用仓库提供的 Supervisor 子智能体，用自然语言明确指定：

```text
请使用 scientific-supervisor 子智能体，遵循 scientific-research-project。
新项目先澄清研究目的；已有项目先核对五个根文件和已批准计划。
如果子智能体需要我的决定，请主会话转达简短问题并等我回答，
不要让子智能体代替我确认。
```

用 `/agents` 查看是否识别 `scientific-supervisor`；若未出现，检查当前项目的 `.claude/agents/scientific-supervisor.md` 与 `.claude/skills/scientific-research-project/SKILL.md`，再重新打开会话。该子智能体通过适配文件中的 `skills` 字段加载科研 Skill。适配文件存在不等于已安装，也不等于启动常驻服务。参见 [Claude Code 技能说明](https://code.claude.com/docs/en/skills)与[子智能体说明](https://code.claude.com/docs/en/sub-agents)。

#### 7.4.3 多个科研技能共存时的职责

| 层次 | 入口 | 职责 |
|---|---|---|
| 总体科研规范 | `scientific-research-project` | 接洽、计划、五个根文件、任务与成果包对应、质控和解释要求 |
| 持续调度 | 常驻 Supervisor；Claude 子智能体可协助核查与操作 | 按已批准依赖与资源推进；子智能体本身不是常驻进程 |
| 专业方法 | 按需使用文献、统计、RDKit、绘图、Word 等技能 | 为计划中的具体任务提供方法与产物，不另起总体规划 |

可以把以下项目约定加入 `AGENTS.md`（Codex）或 `CLAUDE.md`（Claude Code）：

```text
本项目使用 scientific-research-project 作为总体科研流程规范。
新项目/含糊科研方向先分轮交流并等待回答，经确认后才制定草案。
其他技能按需提供专业方法，不另建总体计划或平行结果目录。
已有批准任务遵循 01_PLAN.md 与 registry/tasks.tsv，保留完整研究范围。
```

这是职责约定，不是平台原生的技能优先级机制，也不能覆盖平台权限与更高优先级规则。不必禁用所有其他科研技能；同名旧副本则需核对实际加载路径，不能假定它们会自动合并。单纯解释、安装或文档修改不是新科研项目，不应强行进入接洽。

#### 7.4.4 从对话转入执行

方向确认后先建立两个草案，真实调研并获得正式方案确认后，才建立五个正式根文件与任务表。`project_template/` 是文档起点，不能把空模板、关键词或 Agent 自己写的总结当作用户批准。冻结工具会安装通用项目事件钩子，但不会自动编写领域分析程序。

若希望后台持续推进，可在方案已批准后发送：

```text
请检查并接入本项目的常驻 Supervisor，先核对已有服务和作业，避免重复启动。
在已批准范围内持续调度就绪任务并注册监控，
报告实际服务、正在运行的科研任务、阻断原因和查看进度的方法。
```

调用 Skill 或 Claude 子智能体并不会自动启动常驻调度。服务、真实执行合同、主机、权限和监控仍须按 7.5 配置；当前内置工作程序使用 Codex CLI，不能声称已经验收 Claude 驱动的全链路后台执行。

项目文件与任务注册表准备好后，运行下列检查与审阅命令；替换路径和任务编号：

```bash
research_scripts="$HOME/git/scientific_agent_design/skills/scientific-research-project/scripts"
research_project="/absolute/path/to/project"
python3 "$research_scripts/supervisor_agent.py" intake --root "$research_project"
python3 "$research_scripts/supervisor_agent.py" inspect --root "$research_project"
python3 "$research_scripts/supervisor_agent.py" plan --root "$research_project"
python3 "$research_scripts/supervisor_agent.py" review --root "$research_project" --task-id Q01.01
python3 "$research_scripts/supervisor_agent.py" reflect --root "$research_project" --task-id Q01.01
```

这些命令不会自动建立完整方案，也不会仅凭 `plan` 命令启动全部分析。

### 7.5 持续执行、监控与更新

按[持续执行契约](skills/scientific-research-project/references/continuous_execution.md)准备 `registry/supervisor.json`：依赖、实际命令、环境、主机、资源、哈希、报告与解读阶段；配置项目钩子、报告工具及工作程序。缺少这些内容时，仅安装 Skill 不能自动完成项目。

```bash
research_scripts="$HOME/git/scientific_agent_design/skills/scientific-research-project/scripts"
research_project="/absolute/path/to/project"
python3 "$research_scripts/supervisor_daemon.py" check --root "$research_project"
python3 "$research_scripts/install_supervisor_service.py" --root "$research_project" --start
python3 "$research_scripts/supervisor_daemon.py" status --root "$research_project"
tail -f "$HOME/.codex/monitor/status.md"
```

`--start` 会启动服务与就绪的已授权任务。安装器依赖已存在的 `codex-background-monitor.service`，本仓库没有附带其实现，新电脑须先配置兼容监控。安装器返回项目服务名；用 `systemctl --user stop 服务名` 停止调度，已启动远程作业须单独检查。

项目进度见 `04_STATUS.md`、`provenance/supervisor/status.md`。关机时本地调度停止，恢复后先核对原作业编号与输出；注销后维持服务取决于系统用户驻留设置。修复见[失败恢复规范](skills/scientific-research-project/references/failure_recovery.md)。

更新源码后重新复制 Skill，核对副本再按需重启服务。服务使用安装时记录的脚本绝对路径，只更新另一副本不会自动替换它。

### 7.6 启用严格计划与成果包验收

以下是流程中的依次执行命令，不应一次粘贴运行。先按[工作流执行契约](skills/scientific-research-project/references/workflow_contract.md)准备草案 JSON；真实调研与用户确认后，再准备方案、审批、依赖与主机配置。冻结前不能伪造审批文件来跳过讨论。

```bash
research_scripts="$HOME/git/scientific_agent_design/skills/scientific-research-project/scripts"
research_project="/absolute/path/to/new-project"
python3 -m pip install -r "$research_scripts/requirements-report.txt"
python3 "$research_scripts/research_workflow.py" draft --root "$research_project" --spec "$research_project/provenance/draft.json"
python3 "$research_scripts/research_workflow.py" freeze --root "$research_project" --proposal "$research_project/provenance/proposal.json" --approval "$research_project/provenance/approval.json"
python3 "$research_scripts/research_workflow.py" check --root "$research_project"
python3 "$research_scripts/bootstrap_supervisor.py" --root "$research_project" --dependencies "$research_project/registry/dependencies.tsv" --hosts "$research_project/registry/hosts.json"
```

`bootstrap_supervisor.py` 只生成依赖配置骨架，不会凭空生成可运行的科研命令。补齐并核对阶段契约后，再使用 7.5 的检查与启动命令。报告环境需要 `python-docx`；本次测试使用 Python 3.13、python-docx 1.2.0，图形测试使用 Pillow 12.0.0。

分析工作程序先生成真实结果表、图及报告清单，再生成最终报告；解释阶段写入真实反思证据并更新完整 QC 后，才运行第二条验收命令。示例结果路径必须替换为任务表登记的路径，`SCI_RUN_ID` 为 Supervisor 为该运行分配的编号。

```bash
python3 "$research_scripts/research_workflow.py" render --root "$research_project" --task-id Q01.01 --manifest "$research_project/results/Q01_question/Q01.01_analysis/_evidence/report.json"
python3 "$research_scripts/research_workflow.py" validate-package --root "$research_project" --task-id Q01.01 --run-id "$SCI_RUN_ID"
```

最终报告清单已生成验收收据时，工具拒绝静默覆盖；批次草案不应冒充最终报告。修订须保留旧运行证据并经审查处理。程序不会自行编造科研结论、文献或用户批准。

旧项目不会自动获得这些新门槛。不要对已有项目直接运行 draft/freeze：工具拒绝覆盖已有注册表和钩子。迁移须保留原计划、编号、结果、运行状态和审批记录，审查后接入；不能删除注册表或重跑已完成研究来绕过检查。

## 8. 仓库导航

```text
README.zh-CN.md                    # 中文介绍与安装
README.en.md                       # 对应的英文介绍与安装
VERSION                            # 当前版本号
skills/scientific-research-project/
  SKILL.md                         # 核心行为协议
  references/                      # 深度研究、成果包、恢复与执行规范
  scripts/                         # Supervisor、驱动、检查与测试
adapters/claude/                    # Claude Agent 适配
project_template/                  # 项目文档模板
documentation/                     # 已有架构资料与图形，注意日期版本
archive/                           # 历史资料
```

已有日期版 DOCX、PDF、图形可能反映早期设计；当前说明以两份 README 与对应源码为准。历史材料不作为新增功能已经实现的证明。

## 9. 版本

当前源码版本：**0.6.3**，见 [`VERSION`](VERSION)。保留分轮选项式接洽和深度调研要求，进一步明确核心证据来源、借鉴的科研 Skill，以及旧项目“安装更新—完整替换草稿—审阅—基线迁移”的区别；同步模板与 Supervisor 指令。新冻结会拒绝显式未就绪的调研状态，仍兼容缺少该字段的旧格式。实质研究质量需要真实检索与审阅，不能由状态或结构校验保证。没有新增执行后端或自动迁移命令；更新安装副本不自动迁移或重跑旧项目。
